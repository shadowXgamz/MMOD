...
if (isset($_POST['save'])) {
  ...
  $airplan=$_POST['airplan'];
  $time=$_POST['time'];
  $country=$_POST['country'];
  if ($_POST['save'] == "Save") {
    $save_sql = "INSERT INTO `users`( `name`, `email`, `password`, `mobile`,`airplan`,`time`,`country`) VALUES 
          ('$name','$email','$password','$mobile','$airplan','$time','$country')";
  } else {
    $id = $_POST['id'];
    $save_sql = "UPDATE `users` SET `name`='$name',`email`='$email' ,`mobile`='$mobile',
    `password`='$password',`airplan`='$airplan',`time`='$time',`country='$country'  WHERE id =$id ";
  }
  ...
  <div class="container">
    <div class="wrapper p-5 m-5">
      <div class="d-flex p-2 justify-content-between mb-2">
        <h2>Customers</h2>
        <h3>للتواصل معنا0118435442 0900493184 </h3>
